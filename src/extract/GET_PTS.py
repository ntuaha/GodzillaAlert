...
address
  ...
  def extractData(self,tds):
    ...
    no = tds[0]
    #event type: 道路施工
    data['type'] = self.SOURCE+"_"+tds[1].text
    #熱心聽眾
    data['provider'] = tds[6].text
    if data['provider'] == None:
      data['provider'] = ""
    #地點
    data['title'] = " ".join(tds[2].xpath('.//text()')).replace("\n","").replace("\t","")
    #路況說明
    data['description'] = tds[3].text
    #路況說明: 持續, 排除, 後續
  ...
  def forceCloseEvent(self,id_list):
    ...
    sql = "SELECT event_id FROM event WHERE event_id not in (%s) and status=%d"%(ids,self.RUNNING)
